...
Proactive bot-initiated engagement when the group chat is quiet—silence tracking, Moscow time-window gating, and LLM icebreaker messages.
...
### Requirement: Silence tracking middleware

Silence tracking SHALL run as grammY middleware via `bot.use()` and SHALL call `next()` so downstream message handlers (including the AI reply handler) continue to run.

#### Scenario: Middleware passes through

- **WHEN** a group message update is processed
- **THEN** the initiative middleware SHALL record chat activity
- **AND** SHALL invoke downstream handlers without blocking them
...
When an icebreaker is sent, the bot SHALL call the language model with the `SYSTEM` config as the system message and the `ICEBREAKER` config value as the user message. The call SHALL use the standard `runAgent` path with agent tools enabled.

#### Scenario: Icebreaker uses prompts with tools

- **WHEN** the bot sends an icebreaker
- **THEN** it SHALL read `SYSTEM` and `ICEBREAKER` from `configStore`
- **AND** SHALL invoke `runAgent` with full agent tools available
...
When a user replies to an icebreaker message, the bot SHALL handle the reply using the existing AI trigger rules (`mustBeSendToAI`), matching the bot by `ctx.me` id or username.
